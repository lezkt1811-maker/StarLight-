...
(function (global) {
  ...
  const PLANET_COLOR = {
    Sun: "#d9a636", Moon: "#e8e8e8", Mercury: "#b7b7b7", Venus: "#ffcf7a", Mars: "#ff6a4d",
  ...
  EclipticView.prototype.render = function () {
    ...
    for (const seg of IAU13) {
      ...
      for (const [a, b] of segments) {
        ...
        ctx.fillStyle = alt ? "rgba(217,166,54,0.045)" : "rgba(217,166,54,0.02)";
        ctx.fillRect(a, H * 0.18, b - a, H * 0.5);

        // boundary tick
        ctx.strokeStyle = "rgba(217,166,54,0.28)";
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(a, H * 0.18);
        ctx.lineTo(a, H * 0.68);
        ...
        if (labelX > 0 && labelX < W) {
          ...
          ctx.fillStyle = "rgba(250,209,115, 0.85)";
    ...
    // ecliptic glow line
    ctx.save();
    ctx.shadowColor = "rgba(217,166,54,0.65)";
    ctx.shadowBlur = 8;
    ctx.strokeStyle = "rgba(217,166,54,0.9)";
    ctx.lineWidth = 1.6;
    ctx.beginPath();
    ctx.moveTo(0, eclY);
    ctx.lineTo(W, eclY);
