...
function renderTropicalWheel(combinedChart, targetElement) {
  try {
    ...
    /* Deliberately NOT setting cv.style.width/height here -- the inline
       "width:100%;height:auto" set above already makes the canvas fill
       whatever container it's placed in (the full Tropical tab OR a small
       Compare-view column), and the browser derives the correct height
       from the square raster size below. Setting a hardcoded pixel width
       here previously broke the small Compare preview, whose container
       is narrower than the `css` fallback value. */
...
function sc13ShowChartTab(which) {
  ...
  applyBtnStyle(btnCompare, inactiveStyle);
  ...
  if (which === "tropical") {
    ...
    if (compareDiv) {
      ...
      applyBtnStyle(btnCompare, activeStyle);
      if (typeof window.renderCompareView === "function") {
        window.renderCompareView(window.combinedChart, compareDiv);
      }
    }
  } else {
...
window.sc13OnChartGenerated = function () {
  ...
  } else if (window.sc13ActiveChartTab === "compare") {
    const compareDiv = document.getElementById("compareView");
    if (compareDiv && window.combinedChart && typeof window.renderCompareView === "function") {
      window.renderCompareView(window.combinedChart, compareDiv);
    }
  }
};
